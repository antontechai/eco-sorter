...
pygame.display.set_caption("Eco-sorter") # create a titile for game screen 
...
# !! player_images = [blue_bin_img, white_bin_img, yellow_bin_img]
...
player_color = player_colors[current_color_index] # we get from variable player_colors element with index current_color_index (0,1,2) and save it to variable player_color

# Trash Settings 
# New Structure for type of trash
# TRASH_TYPES = [             # list of dictionary 
#     {"name": "food", "image": food_img, "bin_index": 0}, # name just to make it clear with meaning of paper 
#     {"name": "glass", "image": glass_img, "bin_index": 1}, # color that pygame use to draw the container
#     {"name": "plastic", "image": plastic_img, "bin_index": 2} # bin_index it's right answer, it match the answer so color with player_colors
# ]
# Sort Structure
...
# NEW function for creating new trash subject 
def spawn_trash(): # put code of creating the trash in seperate function, in conviniet purpose because it's neccessary to create new trash once we catch old trash or miss it, instead of repeating code we use spawn_trash()
    ...
    # t = random.choice(TRASH_TYPES) # choose random type of trash from the list TRASH_TYPE
...
trash_list = [] # crreating an open list to containt here all trash objects 
...
while running: # core of the game the code inside the cycle will run again and again 
    ...
    for event in pygame.event.get(): # this command check if player took any actions with keyboard or mouse 
        ...
        if event.type == pygame.KEYDOWN: # checking if there were any interaction with buttons
            if event.key == pygame.K_SPACE:# if yes checking if it was with SPACE
                # updatePlayerColor()
    ...
    trash_spawn_time += 1 # that's timer ticking each frame. When it goes to trash_spawn_interval, we create a new trash(if on the screen there are less than limits) and we reset timer to 0
...
# Logic of player and the trash objects 
    ...
    #screen.fill((0, 0, 0))          # in each frame we clean everything that were drawn in previous frame making black color main 
    ...
    # pygame.draw.rect(screen, trash["type"]["color"], trash["rect"]) # drawing garbage itself color
    for trash in trash_list: # draw each trash from the list 
       screen.blit(trash["image"], trash["rect"])
        # !! pygame.draw.rect(screen, trash["type"]["color"], trash["rect"])   
